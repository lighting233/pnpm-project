...
import { createPageListQuery, createDeleteItem } from '../query/pageContentQuery';
...
// 1.获取List数据,进行展示
const usePageListQuery = createPageListQuery(props.actions.postPageListData)
const { mutate: deleteItem } = createDeleteItem(props.actions.deleteItem, props.contentConfig.pageName)
const {
    pageNO,
    pageCount,
    pageList,
        ...
        beforeClose: (action, instance, done) => {
            try {
                if (action === "confirm") {
                    ...
                    deleteItem({ id: selectedLists.value[0], callback })
                } else {
                    done();
                }
            } catch (error) {
